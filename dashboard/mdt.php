  ...
  <div class="bg-gray-800 rounded-xl p-6 shadow w-full max-w-3xl">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-300">
      <div>
        ...
        <p class="text-lg font-semibold text-white"><?= htmlspecialchars($_SESSION['username'] ?? 'Unknown') ?></p>
      </div>

      <div>
        <p class="text-sm text-gray-400">Department</p>
        <p class="text-lg font-semibold text-white"><?= htmlspecialchars($_SESSION['department'] ?? 'N/A') ?></p>
      </div>

      <div>
        <p class="text-sm text-gray-400">Callsign</p>
        <p class="text-lg font-semibold text-white"><?= htmlspecialchars($_SESSION['callsign'] ?? 'None') ?></p>
      </div>

      <div>
        <p class="text-sm text-gray-400">Status</p>
        <p class="text-lg font-semibold text-green-400">
          <?= htmlspecialchars($_SESSION['status'] ?? '10-7') ?>
        </p>
      </div>
    </div>
  </div>
