...
<aside class="w-64 bg-gray-800 p-4 flex flex-col justify-between fixed h-full">
  <div>
    ...
    <nav class="space-y-2">
      ...
      <a href="file-warrant.php" class="block px-3 py-2 rounded bg-gray-700">File Warrant</a>
...
<main class="ml-64 p-8 w-full bg-gray-900 min-h-screen">
  <div class="max-w-3xl mx-auto">
    <h1 class="text-4xl font-bold mb-6">File Warrant</h1>

    <?php if ($success): ?>
      <p class="text-green-500 mb-4">Warrant filed successfully!</p>
    ...
    <form method="POST" class="space-y-6">
      ...
      <div>
        ...
        <select name="violation" id="violation" onchange="updateFineJail()" required class="w-full px-4 py-2 rounded bg-gray-800">
            ...
            <?php if (!empty($sections_by_title[$t['id']])): ?>
              <optgroup label="<?= htmlspecialchars($t['name']) ?>">
                <?php foreach ($sections_by_title[$t['id']] as $sec): ?>
                  <option 
                    value="<?= htmlspecialchars($sec['code'] . ' - ' . $sec['description']) ?>" 
                    data-fine="<?= $sec['fine'] ?>" 
                    data-jail="<?= $sec['jail_time'] ?>">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="number" name="fine" id="fine" class="w-full px-4 py-2 bg-gray-800 rounded">
        </div>

        <div>
          <label class="block mb-1 font-semibold">Jail Time</label>
          <input type="number" name="jail_time" id="jail_time" class="w-full px-4 py-2 bg-gray-800 rounded">
        </div>
      </div>

      <div>
        <label class="block mb-1 font-semibold">Reason</label>
        <textarea name="reason" class="w-full px-4 py-2 bg-gray-800 rounded"></textarea>
      </div>
      ...
      <button type="submit" class="bg-blue-600 hover:bg-blue-700 px-6 py-2 rounded font-semibold">File Warrant</button>
    </form>
  </div>
</main>
...
<script>
  ...
  function updateFineJail() {
    const selected = document.querySelector('#violation').selectedOptions[0];
